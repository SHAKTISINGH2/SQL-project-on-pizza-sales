...
SELECT 
    ROUND((order_details.quantity * pizzas.price),
            2) AS totel_sales
FROM
    order_details
        JOIN
...
GROUP BY HOUR(order_time);
  ...
  SELECT 
    pt.category, COUNT(p.name) as pizza_count
FROM
    pizzas p
JOIN
    pizza_types pt ON p.type_id = pt.type_id
GROUP BY 
    pt.category
...
select order_date,
sum(revenue) over(order_date) as cum_revenue
from
